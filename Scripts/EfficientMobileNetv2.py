...
# Optimized MobileNetV2 Model
class OptimizedMobileNetV2(nn.Module):
    def __init__(self, num_classes=10, width_multiplier=0.75):
        super(OptimizedMobileNetV2, self).__init__()
        # Initialize MobileNetV2 without pre-trained weights for custom width multiplier
        base_model = mobilenet_v2(pretrained=False, width_mult=width_multiplier)
        feature_dim = int(1280 * width_multiplier)

        self.features = base_model.features
        # Adjust final convolution block
        last_conv = nn.Conv2d(
            in_channels=int(320 * width_multiplier), 
...
def main():
    ...
    model = OptimizedMobileNetV2(num_classes=10, width_multiplier=0.75)
