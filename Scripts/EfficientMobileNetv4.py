...
# Custom Loss (Focal + Label Smoothing)   
...
def LARSMobileNetv4():
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d): # Added SE and ECA Blocks to Conv2d layers
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_acc > best_valid_acc:
            ...
            best_model_path = save_dir / f'best_model_acc_epoch_{epoch+1}.pth'
            torch.save({
                'epoch': epoch + 1,
    ...
    torch.save(model.state_dict(), 'mobilenetv4_latest_model.pth')
    wandb.save('mobilenetv4_latest_model.pth')
